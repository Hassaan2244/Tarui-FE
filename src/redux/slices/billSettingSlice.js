...
export const createSetting = createAsyncThunk(
    "setting/create",
    async ({ icon, name, email, phone, address, preparedBy }, { rejectWithValue }) => {
        try {
            const response = await api.post("/api/bill-setting", { icon, name, email, phone, address, preparedBy });
...
export const updateSetting = createAsyncThunk(
    "setting/update",
    async ({ icon, name, email, phone, address, id, preparedBy }, { rejectWithValue }) => {
        try {
            const response = await api.patch(`/api/bill-setting/${id}`, { icon, name, email, phone, address, preparedBy });
            return response.data;
        } catch (err) {
            return rejectWithValue(
                err.response?.data?.message || err.response?.message || "An error occurred"
